...
for i = 1:parallelSimulations
    if ispc % choose windows binary
        parallelProcesses{1,i} = ['start "" 9>"%lock%',num2str(i),'" ' verboseString ' .\bin\vmc_Windows.exe MCpencilbeam_temp_',num2str(i)];
    elseif ifunix % choose linux binary
        parallelProcesses{1,i} = ['start "" 9>"%lock%',num2str(i),'" ' verboseString ' .\bin\vmc_Linux.exe MCpencilbeam_temp_',num2str(i)];
    end
end
