...
DDD.dose = sigma.^(1/p).*gamma(1/p).*(matRad_cylGauss(-1./p,zeta)./sigma ...
    + matRad_cylGauss(-(1/p)-1,zeta).*(beta/p+vargamma*beta+epsilon/R0) )/ ...
	(sqrt(2*pi)*rho*p*alpha^(1/p)*(1+beta*R0));

	
%	calculate LETd
DDD.LETd_RT = ( 0.1 .* (sigma^(2./p).*gamma(2./p) ...
    .* ( matRad_cylGauss(-2./p,xi) - matRad_cylGauss(-2./p,zeta) ) ...  
    - 2 .* (RT/2.) .^ (2./p) .* exp(-(zeta+xi).^2./8)) ...
	./ (p^2.*alpha^(1./p).*(2./p-1.).*(sigma^(1./p+1.).*gamma(1./p+1) ...
    * ( matRad_cylGauss(-1./p-1.,xi) -  matRad_cylGauss(-1./p-1.,zeta) )  ...
