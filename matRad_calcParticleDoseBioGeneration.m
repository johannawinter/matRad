...
if pln.bioOptimization == true
    ...
    switch SourceOfBioData
        ...
        case 'CNAO'
            baseDataBio =matRadParseBioData([pwd filesep 'database']);
            alphaBetaRatios = [2 10];
            if MultiClass == true
                 % assume that we have 2 tissue classes
                 NumTissueClasses =2;
                 for currTissClass = 1:NumTissueClasses
                     for j= 1:length(baseDataBio)
                            ...
                            tmp=interp1(baseDataBio(j).depths,baseDataBio(j).dEdxA(:,currTissClass),...
                                baseData(j).depths,'linear');
                            tmpNaN = tmp./baseData(index).Z;
                            Idx = find(~isnan(tmpNaN));
                            tmpNaN(isnan(tmpNaN))=tmpNaN(Idx(end));
                            baseData(j).alpha(:,currTissClass) = tmpNaN;
                            
                            
                            tmp=interp1(baseDataBio(j).depths,baseDataBio(j).dEdxB(:,currTissClass),...
                                baseData(j).depths,'linear');
                            ...
                            baseData(j).alphaBetaRatio(currTissClass) =alphaBetaRatios(1,currTissClass);
              ...
              figure,      subplot(221),plot(baseData(1).depths(:,1),baseData(1).beta(:,2),'LineWidth',3),title('115MeV','FontSize',20),grid on, hold on
                                       plot([min(baseData(1).depths(:,1)) max(baseData(1).depths(:,1))],[0.05 0.05],'color','r','LineWidth',3),hold off
                           subplot(222),plot(baseData(20).depths(:,1),baseData(21).beta(:,2),'LineWidth',3),title('178MeV','FontSize',20), grid on, hold on
                                       plot([min(baseData(21).depths(:,1)) max(baseData(21).depths(:,1))],[0.05 0.05],'color','r','LineWidth',3),hold off
                           subplot(223),plot(baseData(60).depths(:,1),baseData(60).beta(:,2),'LineWidth',3),title('277MeV','FontSize',20), grid on, hold on
                                       plot([min(baseData(60).depths(:,1)) max(baseData(60).depths(:,1))],[0.05 0.05],'color','r','LineWidth',3),hold off
                           subplot(224),plot(baseData(121).depths(:,1),baseData(121).beta(:,2),'LineWidth',3),title('398MeV','FontSize',20), grid on, hold on
